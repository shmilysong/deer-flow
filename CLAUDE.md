...
| **topic_guardrail** | `@./deerflow_extensions/topic_guardrail/` | 回答范围限制 + 角色定义外部化（编译后改 role_definition.txt 即可微调回答范围）<br>注入方式：`app.py` try/except + `sitecustomize.py` + `deerflow_entry.py`<br>架构：模板字符串替换（`SYSTEM_PROMPT_TEMPLATE`）而非函数 monkeypatch |
