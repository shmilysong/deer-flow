...
| **topic_guardrail** | `@./deerflow_extensions/topic_guardrail/` | 回答范围限制 + 角色定义外部化<br>注入方式：`boot.py` 统一 Boot Loader（`app.py` + `deerflow_entry.py` + `entrypoint.sh`）<br>架构：模板字符串替换 `SYSTEM_PROMPT_TEMPLATE` |
