...
| **settings-dialog-ext**（SettingsDialog 扩展架构 + 菜单精简） | `settings-dialog.tsx` + `registry.ts` + `workspace-nav-menu.tsx` + `app.py` | ✅ 低 | 3 个前端 + 1 个后端（含 S5：菜单项注释隐藏） |
| **env_settings**（多厂商 API Key） | `app.py`（删 import + 加注入块）| ✅ 极低 | 1 个核心（其余在扩展目录）|

> 注：env_settings 的路由和数据操作（`deerflow_extensions/env_settings/router.py`）全部在扩展目录，零侵入。2026-06-02 的所有 Bug 修复（model 必填、429 检测、日志增强）均不涉及核心源码。
