...
| **Env Settings** (`/api/env-settings`) | `GET /` - list all 7 supported providers' env vars (masked); `PUT /` - update a provider's config (api_key, base_url, model); `DELETE /{provider}` - clear a provider's config; `POST /{provider}/verify` - verify API key via provider's `/v1/models` endpoint. Supported providers: siliconflow, deepseek, moonshot, volcengine, dashscope, minimax, zhipuai |
